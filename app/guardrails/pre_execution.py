...
class PreExecutionGuardrail:
    ...
    JAILBREAK_PATTERNS = [
        ...
        r"(show|print|output|display)\s+(the\s+|your\s+)?(system\s+prompt|developer\s+prompt|developer\s+instructions|initial\s+instructions|system\s+instructions)",
        r"disregard\s+(all\s+)?(safety|security|content)\s+filters?",
